...
const Chat = ({ chatSelecionado, socket }) => {
    const nombreDelUsuarioConectado = useRef(window.localStorage.getItem("usuario"));
    ...
    const enviarMensaje = () => {
        if (inputMensaje.current.value !== "") {
            const messageData = {
                ...
                nombreUsu: nombreDelUsuarioConectado.current,
    ...
    useEffect(() => {
        if (chatSelecionado != null) {
            joinRoom(chatSelecionado.nombreChat, nombreDelUsuarioConectado.current);
        }
            ...
            {chatSelecionado != null ? (
                <>
                    ...
                    <div className="chat-input">
                        <input ref={inputMensaje} type="text" placeholder="Escribe un mensaje" />
                        <button onClick={enviarMensaje}>Enviar</button>
                    </div>
                </>
            ) : (
                <div className="no-chat-selected">
                    <img src="https://mailrelay.com/wp-content/uploads/2020/07/How-to-Enhance-Your-Customer-Experience-using-Live-Chat.jpg" alt="Icono de chat" className="chat-icon" />
