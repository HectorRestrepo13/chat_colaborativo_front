...
const Login = ({ setNevegarPagina }) => {
    ...
    if (inputUsuario.current.value !== "" && inputContra.current.value !== "") {
      try {
        const respuesta = await fetch("/api/login/iniciarSesion", {
          ...
          body: JSON.stringify({
            usuario_red: inputUsuario.current.value,
        ...
        if (!respuesta.ok) {
          throw new Error(
            `Hubo un Error al Iniciar Sesión: ${respuesta.status}`
          );
        }
        ...
        if (jsonRespuesta.status == true) {
          setDatoUsuario(jsonRespuesta.descripcion);
  ...
  return (
    <div className="wrapper">
      ...
      <div className="text-center mt-4 name">ABAI GROUP</div>
      <form className="p-3 mt-3">
        ...
        <button type="button" onClick={iniciarSesion} className="btn mt-3">
          Iniciar
        </button>
      </form>
